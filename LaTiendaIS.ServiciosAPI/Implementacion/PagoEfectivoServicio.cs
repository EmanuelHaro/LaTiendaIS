...
namespace LaTiendaIS.ServiciosAPI.Implementacion
{
    public class PagoEfectivoServicio: IPagoEfectivoServicio
    {
        ...
        public PagoEfectivoServicio(IUnitOfWork unitofwork, IMapper mapper)
        {
        ...
        public async Task<bool> AgregarPago(PagoEfectivo Pago)
        {
            try
            {
                var dbPago = _mapper.Map<PagoEfectivoDTO>(Pago);


                var respModelo = await _unitofwork.Repository<PagoEfectivoDTO>().Crear(dbPago);
        ...
        public async Task<Pago> ObtenerPago(int idPago)
        {
            try
            {
                var dbPago = await _unitofwork.Repository<PagoEfectivoDTO>().Obtener(c => c.IdPago == idPago).FirstOrDefaultAsync(); //IGenericoRepositio<ArticuloDTO>.Obtener()  _modeloRepositorio.Obtener()
